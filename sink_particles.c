...
void reinject_gas_mass(int isink)
{
...
#ifdef MHD
      SphP[idx].Energy += 0.5 * (SphP[idx].B[0] * SphP[idx].B[0] + SphP[idx].B[1] * SphP[idx].B[1] + SphP[idx].B[2] * SphP[idx].B[2]) * volume;
#endif
  ...
  if (isnan(SinkP[isink].Mass))
    printf("SinkP[isink].Mass=%g, additional_mass=%g, total_n=%g\n", SinkP[isink].Mass, additional_mass, total_n);
