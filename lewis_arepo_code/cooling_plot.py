...
def plot_alt(file):
	...
	mask=np.where(r>0)
	x,y,z=np.histogram2d(np.log10(abs(a.u/np.sum(a.cooling[:,heat],1))[mask]*code_units.t_cu/(60*60*24*365)),np.log10(r[mask]*code_units.d_cu),bins=(800,800))
	line1=plt.imshow(x/x,cmap='autumn',aspect='auto',label=r'$t_h$',extent=[z[0],z[-1],y[-1],y[0]])
	x,y,z=np.histogram2d(np.log10(abs(a.u/np.sum(a.cooling[:,cool],1))[mask]*code_units.t_cu/(60*60*24*365)),np.log10(r[mask]*code_units.d_cu),bins=(800,800))
	line2=plt.imshow(x/x,cmap='winter',aspect='auto',label=r'$t_c$',extent=[z[0],z[-1],y[-1],y[0]])
	plt.ylim(y[0],y[-1])
	#rho=10**np.linspace(z[0],z[-1],100)
	#tff=np.sqrt(3*np.pi/(32*ap.G.cgs.value*rho))/(60*60*24*365)
...
def both(file):
	...
	axs[0].set_xlabel(r'log$_{10}(\rho$ [gcm$^{-3}$])',fontsize=20)
	axs[0].set_ylabel(r'log$_{10}(t$ [yrs])',fontsize=20,rotation=90)
	...
	mask=np.where(r>0)
	x,y,z=np.histogram2d(np.log10(abs(a.u/np.sum(a.cooling[:,heat],1))[mask]*code_units.t_cu/(60*60*24*365)),np.log10(r[mask]*code_units.d_cu),bins=(800,800))
	line1=axs[1].imshow(x/x,cmap='RdYlBu',aspect='auto',label=r'$t_h$',extent=[z[0],z[-1],y[-1],y[0]])
	x,y,z=np.histogram2d(np.log10(abs(a.u/np.sum(a.cooling[:,cool],1))[mask]*code_units.t_cu/(60*60*24*365)),np.log10(r[mask]*code_units.d_cu),bins=(800,800))
	line2=axs[1].imshow(x/x,cmap='winter',aspect='auto',label=r'$t_c$',extent=[z[0],z[-1],y[-1],y[0]])
	axs[1].set_ylim(y[0],y[-1])
	#rho=10**np.linspace(z[0],z[-1],100)
	#tff=np.sqrt(3*np.pi/(32*ap.G.cgs.value*rho))/(60*60*24*365)
	line3=axs[1].plot(np.log10(rs),np.log10(tff),'k',label=r'$t_{ff}$')
	line1=Line2D([0], [0], color='brown', lw=2)
	line2=Line2D([0], [0], color='b', lw=2)
	line3=Line2D([0], [0], color='k', lw=2)
	axs[1].legend([line1,line2,line3],(r'$t_h$',r'$t_c$',r'$t_{ff}$'),fontsize=12,frameon=False,markerscale=10,loc='upper left')
	...
	axs[1].set_xlabel(r'log$_{10}$(R [cm])',fontsize=20)
